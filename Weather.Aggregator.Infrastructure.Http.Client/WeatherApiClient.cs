...
public sealed class WeatherApiClient(
    ...
    ILogging<WeatherApiClient> logging,
    IBenchmarkService benchmarkService)
    : HttpClientBase(httpClientFactory,  benchmarkService), IWeatherClient
{  
    public override string Name => nameof(WeatherApiClient);
    ...
    {
        var requestUri = $"v1/current.json?q={latitude},{longitude}";
        ...
        try
        {
            ...
            {
                logging.Error("Failed to fetch weather data from Weather API");
            ...
            if (json is null)
            {
                logging.Error("Failed to fetch weather data are empty from Weather API");

                return Result.Failure<CurrentWeather>(ApplicationErrors.WeatherDataFetchFailed);
            }

            logging.Info($"{nameof(WeatherApiClient)}.{nameof(GetWeatherDataAsync)}", new Dictionary<string, object>
            {
                ["ClientName"] = Name,
    ...
    private Result<CurrentWeather> ParseResponse(dynamic json)
    {
        return new CurrentWeather
        {
            ...
            TemperatureC = json.GetProperty("current").GetProperty("temp_c").GetDouble(),
            FeelsLikeC = json.GetProperty("current").GetProperty("feelslike_c").GetDouble(),
            HumidityPercent = json.GetProperty("current").GetProperty("humidity").GetDouble(),
            Condition = json.GetProperty("current").GetProperty("condition").GetProperty("text").GetString()
        };
    }
}